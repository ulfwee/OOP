...
using TL = TheaterLibrary;
namespace lr1_3
{
    public partial class Form1 : Form
    {
        private TL.Group allShowsGroup = new TL.Group();
        private TL.Group filteredGroup = new TL.Group();
        private readonly TL.TheaterFactory factory = new TL.TheaterFactory();
        BindingList<TL.OperaTheater> operaShows = new BindingList<TL.OperaTheater>();
        BindingList<TL.PuppetTheater> puppetShows = new BindingList<TL.PuppetTheater>();

        private Action<TL.Theater> notify;
        ...
        public Form1()
        {
            ...
            rowMenu.Items.Add("Видалити", null, DeleteRow_Click);
        ...
        private void LogToConsole(TL.Theater t)
        {
            Console.WriteLine("Додано: " + t.Name);
        }


        //private void OnTheaterAdded(TL.Theater t)
        //{
        //    try
        //    {
        ...
        private void OnGroupCleared(object? sender, TL.TheaterEventArgs e)
        {
            MessageBox.Show(e.Message);
        }


        private Predicate<TL.Theater> FilterByGenre(string genre) =>
            t => t.Genre == genre;

        private Predicate<TL.Theater> FilterByDate(DateTime from, DateTime to) =>
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (type == "Ляльковий")
            {
                ...
                var puppetRating = puppet.RatePuppet();
                MessageBox.Show(puppetRating);
            ...
            else if (type == "Оперний")
            {
                ...
                var operaRating = opera.RateOpera();
                MessageBox.Show(operaRating);
        ...
        private void label4_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (category == "Ляльковий")
                    SaveShowsToFile(dialog.FileName, puppetShows.Cast<TL.Theater>());
                else
                    SaveShowsToFile(dialog.FileName, operaShows.Cast<TL.Theater>());
        ...
        {
            IEnumerable<TL.Theater> filtered = allShowsGroup;
        ...
        private void label15_Click(object sender, EventArgs e)
        {
            ...
            if (open.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    if (category == "Ляльковий")
                    {
                        var list = JsonSerializer.Deserialize<List<TL.PuppetTheater>>(vmist);
                        if (list != null)
                    ...
                    {
                        var list = JsonSerializer.Deserialize<List<TL.OperaTheater>>(vmist);
                        if (list != null)
        ...
        private void UpdateDataGridView(IEnumerable<TL.Theater> shows)
        {
            var displayList = shows.Select(show => new
            {
                ...
                Date = show.Date,
                PuppetType = show is TL.PuppetTheater puppet ? puppet.PuppetType : null,
                AgeCategory = show is TL.PuppetTheater puppet2 ? puppet2.AgeCategory : null,
                Composer = show is TL.OperaTheater opera ? opera.Composer : null,
                Language = show is TL.OperaTheater opera2 ? opera2.Language : null,
                Subtitles = show is TL.OperaTheater opera3 ? (opera3.Subtitles ? "Так" : "Ні") : null,
            ...
            if (dataGridView1.Columns["Original"] != null)
                dataGridView1.Columns["Original"].Visible = false;
        }
        ...
        private void SaveShowsToFile(string path, IEnumerable<TL.Theater> shows)
        {
        ...
        private void dataGridView1_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (hit.RowIndex >= 0)
                {
                    ...
                    rowMenu.Show(dataGridView1, e.Location);
                }
            }
        }
        ...
        private void DeleteRow_Click(object? sender, EventArgs e)
        {
            ...
            {
                dynamic row = dataGridView1.Rows[clickedRowIndex].DataBoundItem;
                TL.Theater item = row.Original;   

                allShowsGroup.Remove(item);
